<?php
//API cho bài viết
//Xử lý các thao tác CRUD cho bài viết, bao gồm chủ đề nổi bật, bài viết gần đây và lọc theo danh mục
...
// Lấy action từ query string - với action mặc định
...
function handleUpdatePost($post, $requestData, $getParams) {
    ...
    if ($currentPost['data']['user_id'] !== $_SESSION['user_id'] && (!isset($_SESSION['role']) || $_SESSION['role'] !== 'admin')) {
